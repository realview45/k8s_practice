...
//config와 filter로직을 한번에 처리하고있다.구리
@Component
public class JwtAuthFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        String urlPath = exchange.getRequest().getURI().getRawPath();// /member/create만 꺼낸다. yml에 설정해놨기때문에
...
//            admin권한 있어야 하는 url 검증
            if (ADMIN_ONLY_PATH.contains(urlPath) && !role.equals("ADMIN")) {//권한에러시 403
